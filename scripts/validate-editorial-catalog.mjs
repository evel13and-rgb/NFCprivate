...
if (safeIntermediate.length !== 506) {
  blockingErrors.push(`quotes.intermediate.json: se esperaban 506 frases y hay ${safeIntermediate.length}`);
}
if (safeNormalized.length !== 506) {
  blockingErrors.push(`quotes.normalized.draft.json: se esperaban 506 frases y hay ${safeNormalized.length}`);
}
if (migrationReport?.total_quotes !== 506) {
  blockingErrors.push(`migration-report.json: total_quotes debe ser 506 y es ${migrationReport?.total_quotes}`);
}
if (normalizationReport?.total_quotes_processed !== 506) {
  blockingErrors.push(
    `normalization-report.json: total_quotes_processed debe ser 506 y es ${normalizationReport?.total_quotes_processed}`,
  );
}
