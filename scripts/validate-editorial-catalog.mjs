...
if (safeIntermediate.length !== 572) {
  blockingErrors.push(`quotes.intermediate.json: se esperaban 572 frases y hay ${safeIntermediate.length}`);
}
if (safeNormalized.length !== 572) {
  blockingErrors.push(`quotes.normalized.draft.json: se esperaban 572 frases y hay ${safeNormalized.length}`);
}
if (migrationReport?.total_quotes !== 572) {
  blockingErrors.push(`migration-report.json: total_quotes debe ser 572 y es ${migrationReport?.total_quotes}`);
}
if (normalizationReport?.total_quotes_processed !== 572) {
  blockingErrors.push(
    `normalization-report.json: total_quotes_processed debe ser 572 y es ${normalizationReport?.total_quotes_processed}`,
  );
}
