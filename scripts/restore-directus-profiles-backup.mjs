...
async function main() {
  ...
  if (options.help) {
    process.stdout.write('Uso: node scripts/restore-directus-profiles-backup.mjs --backup=<ruta> [--restore y confirmaciones]\n');
  ...
  validatePublicProfilesDocument(JSON.parse(backupText));
  validatePublicProfilesDocument(JSON.parse(currentText));
  ...
  const errors = [];
  if (options.confirmAction !== 'RESTORE_PROFILES') {
    errors.push('falta --confirm-action=RESTORE_PROFILES');
  }
  ...
  const replacement = await atomicReplaceArtifact({
    ...
    operationId: `restore-profiles-${Date.now()}`,
    targetPath,
  });
  process.stdout.write(`${JSON.stringify({
    mode: 'restore',
