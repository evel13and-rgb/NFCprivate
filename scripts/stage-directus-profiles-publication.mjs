...
} from './prepare-directus-profiles-publication.mjs';
...
const DEFAULT_CANDIDATE_PATH = '/tmp/paramo-directus-profiles-publication-candidate.json';
const BACKUP_DIRECTORY = '/var/lib/paramo-directus/publication-backups';
const scriptDirectory = path.dirname(fileURLToPath(import.meta.url));
const projectRoot = path.resolve(scriptDirectory, '..');
const targetPath = path.join(projectRoot, 'public', 'data', 'literary-profiles.json');
...
function validateSourceRun(
  ...
  if (run.artifact_hashes?.artifact_kind !== 'literary_profiles') {
    errors.push('artifact_kind debe ser literary_profiles');
  }
  if (run.artifact_hashes?.profiles_candidate_sha256 !== candidateSha) {
  ...
  if (run.entity_counts?.authors !== candidateDocument.authors.length) {
    errors.push('el recuento de autores no coincide con publication_runs');
  }
  if (run.entity_counts?.works !== candidateDocument.works.length) {
    errors.push('el recuento de obras no coincide con publication_runs');
  }
  const quoteCount = candidateDocument.works.reduce((total, work) => (
    total + work.fragment_count
  ), 0);
  if (run.entity_counts?.profile_fragments !== quoteCount) {
    errors.push('el recuento de fragmentos no coincide con publication_runs');
  }
  if (repositoryCommit && run.git_commit !== repositoryCommit) {
    errors.push('el commit actual no coincide con el publication_run de vista previa');
  }
...
function validateStageConfirmations(options, { candidateSha, currentPublicSha }) {
  const errors = [];
  if (options.confirmAction !== 'STAGE_PROFILES') {
    errors.push('falta --confirm-action=STAGE_PROFILES');
  }
...
async function main() {
  ...
  if (options.help) {
    process.stdout.write('Uso: node scripts/stage-directus-profiles-publication.mjs --run=<uuid> [--stage y confirmaciones]\n');
    process.stdout.write('Prepara literary-profiles.json en el repositorio; nunca despliega /var/www.\n');
    return;
  }
  ...
  const comparison = {
    ...contentComparison,
    byte_exact: candidateText === currentPublicText,
    author_count_before: currentPublicDocument.authors.length,
    author_count_after: candidateDocument.authors.length,
    work_count_before: currentPublicDocument.works.length,
    work_count_after: candidateDocument.works.length,
  };
  const contentChange = contentChangeDecision({
    ...comparison,
    exact: comparison.exact && comparison.byte_exact,
  ...
  try {
    ...
    const productionRun = await request('/items/publication_runs?fields=id,status', {
      ...
      body: JSON.stringify({
        ...
        errors: [],
        git_commit: repository.commit,
        notes: `Fichas preparadas desde la vista previa ${sourceRun.id}; despliegue web pendiente.`,
      }),
    });
    ...
    await updateProductionRun({
      ...
      artifact_hashes: {
        ...
        staged_profiles_sha256: candidateSha,
        backup_sha256: replacementResult.backup_sha256,
      },
      finished_at: new Date().toISOString(),
      notes: `Fichas preparadas desde ${sourceRun.id}. Copia reversible: ${replacementResult.backup_path}. Despliegue web pendiente.`,
    });
