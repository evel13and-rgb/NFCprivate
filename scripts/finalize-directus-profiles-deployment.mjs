...
import { validatePublicProfilesDocument } from './export-directus-profiles-preview.mjs';
...
const SERVED_PROFILES_URL = 'https://paramoliterario.com/public/data/literary-profiles.json';
const scriptDirectory = path.dirname(fileURLToPath(import.meta.url));
const projectRoot = path.resolve(scriptDirectory, '..');
const sourcePath = path.join(projectRoot, 'public', 'data', 'literary-profiles.json');
const deployedPath = '/var/www/paramo-literario/public/data/literary-profiles.json';
...
function validateProductionRun(run) {
  ...
  if (!/^[a-f0-9]{64}$/u.test(run.artifact_hashes?.staged_profiles_sha256 || '')) {
    errors.push('falta staged_profiles_sha256');
  }
  for (const entity of ['authors', 'works']) {
    if (!Number.isInteger(run.entity_counts?.[entity]) || run.entity_counts[entity] < 1) {
      errors.push(`entity_counts.${entity} no es válido`);
    }
  }
...
function validateFinalizeConfirmations(options, expectedSha) {
  const errors = [];
  if (options.confirmAction !== 'FINALIZE_PROFILES') {
    errors.push('falta --confirm-action=FINALIZE_PROFILES');
  }
...
async function gitState() {
  const [{ stdout: commitOutput }, { stdout: statusOutput }, { stdout: trackedText }] = await Promise.all([
    ...
    execFileAsync('git', ['show', 'HEAD:public/data/literary-profiles.json'], {
      cwd: projectRoot,
  ...
  return {
    ...
    tracked_profiles_sha256: sha256(trackedText),
  };
}
...
async function main() {
  ...
  if (options.help) {
    process.stdout.write('Uso: node scripts/finalize-directus-profiles-deployment.mjs --run=<uuid-producción> [--finalize y confirmaciones]\n');
  ...
  try {
    ...
    const expectedSha = run.artifact_hashes.staged_profiles_sha256;
    if (repository.tracked_profiles_sha256 !== expectedSha) {
      throw new Error('El JSON versionado en HEAD no coincide con el production_run');
    }
    const servedUrl = `${SERVED_PROFILES_URL}?publication_run=${encodeURIComponent(run.id)}&t=${Date.now()}`;
    const verification = await verifyDeployedJson({
      ...
      expectedEntityCounts: {
        authors: run.entity_counts.authors,
        works: run.entity_counts.works,
      },
      expectedSha,
      servedUrl,
      sourcePath,
    ...
    const report = {
      ...
      tracked_profiles_sha256: repository.tracked_profiles_sha256,
      deployed_path: deployedPath,
      served_url: SERVED_PROFILES_URL,
    ...
    if (run.status !== 'published') {
      await request(`/items/publication_runs/${run.id}?fields=id,status`, {
        ...
        body: JSON.stringify({
          ...
          notes: `${run.notes || ''}\nDespliegue de fichas verificado en disco y por HTTPS; estado published confirmado.`.trim(),
        }),
      });
    }
    process.stdout.write(`${JSON.stringify(report, null, 2)}\n`);
